...
export class MapComponent implements OnInit {
  ...
  */
    ...
    url : 'http://localhost:8080/tilesets/Terschelling'
  });
  ...
  */
  createCesiumMap():void{
    ...
    this.map = new Cesium.Viewer('map',
     {
        ...
        //imageryProvider: new CesiumImageryProvider({url: '//a.tile.openstreetmap.org/'}),
        //requestRenderMode : true,
        //terrainProvider : Cesium.createWorldTerrain(),
        terrainProvider : new Cesium.CesiumTerrainProvider({
          url : 'http://localhost:8080/tilesets/Hamert-DTM'
        }),
        timeline: false,
        animation: false,
    ...
    // /*
    // FIX THE CAMERA CLIPPING BELOW THE GROUND WHEN LOADING
    // */
    // this.map.camera.changed.addEventListener(
    //   function() {
    //     if (_this.map.camera._suspendTerrainAdjustment && _this.map.scene.mode === Cesium.SceneMode.SCENE3D) {
    //     _this.map.camera._suspendTerrainAdjustment = false;
    //     _this.map.camera._adjustHeightForTerrain();
    //     }
    //   }
    // );
  };
  ...
  removeItem(item:Item):void{
    ...
    */
    ...
    item.layerGroups.forEach(layerGroup => {
      this.map.scene.primitives.remove(layerGroup);
    });
  ...
  */
  ...
  /*
    ...
    */
    addLayerGroup(item:Item):void{
          ...
          let lineLayer = new Cesium.PolylineCollection();

          if(!item.layerGroups.has(layerGroupSelector)){

            for (let i = 1; i < item.coordinateList.length; i++) {

              let point1 = Cesium.Cartesian3.fromDegrees(
                item.coordinateList[i-1][0],
                item.coordinateList[i-1][1],
                (item.altitudeList[i-1]+3))

              let point2 = Cesium.Cartesian3.fromDegrees(
                item.coordinateList[i][0],
      				 	item.coordinateList[i][1],
      				 	(item.altitudeList[i]+3))
            ...
            this.map.scene.primitives.add(lineLayer);
  ...
  removeLayerGroup(layerGroupKey:string):void{
    ...
    this.map.scene.primitives.remove(groupToRemove);
